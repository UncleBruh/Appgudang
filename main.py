...
class Window(tk.Tk):
	...
	def create_Createaccount(self):
		self.pages['Createaccount'] = createaccount(self.container, self)
	...
	def show_about_info(self):
		msg.showinfo("About Contact App", "WRYYYYYYY")
	...
	def clicked_save_register_btn(self):
		...
		if confirm:
			registered_username = self.pages['Createaccount'].var_register_username.get()
			registered_password = self.pages['Createaccount'].var_register_password.get()

			if registered_username in self.settings.users:
				msg.showerror("Registration Error", "Use another username")
				self.create_Createaccount()
			if registered_username == "":
				msg.showerror("Registration Error", "There's no Password")
				self.create_Createaccount()
			if registered_password == "":
				msg.showerror("Registration Error", "You do remember the password right?")
				self.create_Createaccount()
			else:
				new_acc = {
					registered_username : {
